...
// Start coding here
const countMembers = bills.reduce((totalAcc, bill) => {
  if (bill.member) {
    totalAcc.push(bill.member.name);
  }
  return totalAcc;
}, []);
const totalMembers = countMembers.filter((member, index) => {
  return countMembers.indexOf(member) === index;
});
